...
def _parse_response(response: str) -> FileReviewResponse:
    try:
...
def _calculate_score(comments: list[ReviewComment]) -> int:
    PENALTIES = {
        Severity.INFO: 1,
        Severity.WARNING: 4,
        Severity.ERROR: 10,
...
def _calculate_has_critical_issues(comments: list[ReviewComment]) -> bool:
...
class Analyzer:
    ...
    def analyze_pr(self, pr: PR, settings: Settings) -> ReviewResult:
        ...
        score = _calculate_score(all_comments)

        has_critical_issues = _calculate_has_critical_issues(all_comments)
